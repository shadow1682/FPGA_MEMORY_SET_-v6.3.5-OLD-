...
BOOL CFPGA_Memory_SetDlg::OnInitDialog()
{
	...
	SetDlgItemText(IDC_RADIO_RSA, "Rsa:0x30000-0x30FFF(4K)");
...
unsigned char buffer_PROM[ADD_LEN] = { 0 }, buffer_ATMC[ADD_LEN] = { 0 }, buffer_TROM[ADD_LEN] = { 0 }, buffer_RSA[ADD_LEN] = { 0 };
...
void CFPGA_Memory_SetDlg::OnBnClickedButtonRam()
{
	...
	else if (((CButton*)GetDlgItem(IDC_RADIO_TROM))->GetCheck())
	{
		check_PROM = FALSE;
		check_TROM = TRUE;
		check_ATMC = FALSE;
...
DWORD WINAPI ThreadFun(LPVOID pArg)						//线程入口函数
{
	...
	int tras_buf1_RSA = 0, tras_buf2_RSA = 0, tras_add_RSA = 0, tras_buf0_RSA = 0;

	int m_2 = 0, m_1_TROM = 0, m_1_PROM = 0, m_1_ATMC = 0, m_1_RSA = 0,m_all = 0;
	...
	if(cMyDlg->m_combox_form.GetCurSel() == FILE_RCF_ENC)//RCF文件
	{
		...
		if (cMyDlg->check_RSA == TRUE)//RSA
		{
			...
			while (file.ReadString(strText))
			{
				...
				for (m_1_RSA = 4; m_1_RSA > 0; m_1_RSA--)//将数据存入buffer中
				{
					buffer_RSA[tras_add_RSA + 0x30000] = trans_buffer[m_1_RSA - 1];
...
Step3: 		if (cMyDlg->check_ATMC == TRUE)
	...
	else if (cMyDlg->check_TROM == TRUE)
	{
	...
	{
		for (int h = 0; h<ADD_LEN_RSA; h += LEN_BUF_STR)
		{
			memcpy(&trs_buffer[0], &buffer_RSA[h + 0x30000], LEN_BUF_STR);
	...
	{
		while (m_all<ADD_LEN_RSA)//发送数据
		{

			for (m_2 = 0; m_2<SEND_SIZE; m_2++)//64个数据一发
			{
				send_buffer[m_2] = buffer_RSA[m_all + 0x30000];
				m_all++;
			}
...
void CFPGA_Memory_SetDlg::OnBnClickedButtonRead()
{
	...
	{
		memory[0] = 0x0D;
		if (m_Blen > 4 || m_Blen < 1)
		{
			MessageBox("读取TROM数据大小超出范围（1~4K）", "ERROR!", MB_ICONEXCLAMATION);
			return;
		}
	}
...
DWORD WINAPI ThreadFun2(LPVOID pArg2)						//线程入口函数
{
	...
	unsigned char recv_buffer_PROM[ADD_LEN_PROM] = {0}, recv_buffer_ATMC[ADD_LEN_ATMC] = { 0 }, 
					recv_buffer_TROM[ADD_LEN_TROM] = { 0 }, recv_buffer_RSA[ADD_LEN_RSA] = { 0 },buffer[128] = {0};
	...
	else if(cMyDlg2->check_TROM == TRUE)
	{
	...
	{
		USBBulkReadData(0, EP1_IN, (char*)recv_buffer_RSA, cMyDlg2->m_Blen * 1024, 4000);

		while (m_1<(cMyDlg2->m_Blen) * 1024)
		{
			str_trs = transResult(&recv_buffer_RSA[m_1], 64);
...
void CFPGA_Memory_SetDlg::OnBnClickedButtonClrSendedit()
{
	...
	else if (check_TROM == TRUE)memset(buffer_TROM, 0, ADD_LEN_TROM);
	else memset(buffer_RSA, 0, ADD_LEN_RSA);
...
void CFPGA_Memory_SetDlg::Func_Enable(bool bEnable)
{
	...
	::EnableWindow(::GetDlgItem(this->GetSafeHwnd(), IDC_RADIO_RSA), bEnable);
